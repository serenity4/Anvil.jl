...
Layout.set_position!(storage::ECSLayoutStorage{C}, object::EntityID, position::C) where {C} = storage.ecs[object, LOCATION_COMPONENT_ID] = position
...
Layout.set_geometry!(storage::ECSLayoutStorage{<:Any,GC}, object::EntityID, geometry::GC) where {GC} = storage.ecs[object, GEOMETRY_COMPONENT_ID] = geometry
