...
function BasicRenderer(instance_extensions, device_features::PhysicalDeviceFeatures, device_extensions, window::AbstractWindow)
    ...
    device, device_ci = init(; instance_extensions, device_extensions, enabled_features = device_features, nqueues = 1)
...
submit(r::AbstractRenderer, submits::AbstractArray{<:SubmitInfo2KHR}; fence = C_NULL) =
    queue_submit_2_khr(get_device_queue_2(r.device, r.queue), submits, function_pointer(r.device, "vkQueueSubmit2KHR"); fence)
