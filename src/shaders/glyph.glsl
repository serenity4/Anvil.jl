...
layout(location = 0) in vec4 vcolor;      // Vertex color.
layout(location = 1) in vec2 texcoord;    // Em-space glyph coordinates for pixel.
layout(location = 2) in uvec2 glyph_param; // Constant data for current glyph.
layout(location = 3) in vec3 band_param;   // Scale and offset for band indexes.

layout(binding = 1) uniform sampler2D curve_tex;
layout(binding = 2) uniform sampler2D band_tex;
...
void main()
{
    ...
    // The low 12 bits of each component of glyph_param give the coordinates
    ...
    uvec2 glyphLoc = glyph_param & 0x0FFFU;
    uvec2 bandMax = glyph_param >> 12U;

    // Determine what bands the current pixel lies in by applying a scale
    // and offset to the texture coordinates. The scale given by band_param.z
    // is the same for both directions, but there are different offsets given
    // by band_param.xy. Band indexes are clamped to [0, bandMax.xy].

    uvec2 bandIndex = uvec2(clamp(ivec2(texcoord * band_param.z
                        + band_param.xy), ivec2(0U, 0U), ivec2(bandMax)));
    ...
    uvec2 hbandData = texelFetch(band_tex,
    ...
    uvec2 vbandData = texelFetch(band_tex,
    ...
    out_color = vec4(vcolor.xyz * alpha, alpha);
}
