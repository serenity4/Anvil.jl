function Render.command_buffers(rdr::BasicRenderer, frame::FrameState, app::ApplicationState)
    ...
    @record command_buffer begin
        ...
        cmd_begin_render_pass(
            RenderPassBeginInfo(
                ...
                Rect2D(Offset2D(0, 0), Extent2D(app.resolution...)),
                [vk.VkClearValue(vk.VkClearColorValue((0.1, 0.1, 0.1, 0.1)))]
            ),
...
function create_pipeline(rdr::BasicRenderer, rstate::RenderState, app::ApplicationState)
    @unpack device = rdr
    ...
    viewport_state = PipelineViewportStateCreateInfo(
        viewports = [Viewport(app.position..., app.resolution..., 0, 1)],
        scissors = [Rect2D(Offset2D(0, 0), Extent2D(app.resolution...))],
    )
